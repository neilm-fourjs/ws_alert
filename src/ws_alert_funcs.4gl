#+ API
#+ AlertService/alert/<type>/<message> - raise an alert
#+ AlertService/status  - just return the status of the alert service
#+ AlertService/maint/start/<reason>  - service into maintenance mode - alerts are ignored
#+ AlertService/maint/stop  - service back to normal
#+ AlertService/exit  - just exit the service
#+
#+ http://localhost/g/ws/r/ws_alert/AlertService/exit
...
PUBLIC FUNCTION start()
	...
	WHILE TRUE
		...
		IF int_flag != 0 THEN
			...
			EXIT WHILE
		END IF
		IF m_stop THEN EXIT WHILE END IF
	END WHILE
...
PUBLIC FUNCTION exit() ATTRIBUTES(WSGet, WSPath = "/exit", WSDescription = "Exit the service") RETURNS STRING
...
-- check the status in the database
	TRY
		SELECT state, state_desc, last_update_on, last_update_at INTO l_stat, l_statDesc, l_date, l_time FROM alert_status
	...
	RETURN service_reply(l_reply)
END FUNCTION
----------------------------------------------------------------------------------------------------
-- Add an alert to the alert table
PUBLIC FUNCTION alert(l_type STRING ATTRIBUTES(WSParam), l_message STRING ATTRIBUTES(WSParam))
		ATTRIBUTES(WSGet, WSPath = "/alert/{l_type}/{l_message}", WSDescription = "Returns status of service")
		RETURNS STRING
	DEFINE l_reply STRING
	DEFINE l_stat CHAR(1)
...
PUBLIC FUNCTION maintStart(l_reason STRING ATTRIBUTES(WSParam))
		ATTRIBUTES(WSGet, WSPath = "/maint/start/{l_reason}", WSDescription = "Returns status of service") RETURNS STRING
	DEFINE l_reply STRING
	DEFINE l_date DATE
	DEFINE l_time DATETIME HOUR TO SECOND
...
-- Update the status in the database
	TRY
		UPDATE alert_status SET (state, state_desc, last_update_on, last_update_at) = ("M", l_reason, l_date, l_time)
				WHERE controller_name = m_controller
	CATCH
	END TRY
	IF STATUS != 0 THEN
...
PUBLIC FUNCTION maintStop(
		) ATTRIBUTES(WSGet, WSPath = "/maint/stop", WSDescription = "Take service out of maintenace mode.") RETURNS STRING
	DEFINE l_reply STRING
	DEFINE l_date DATE
	DEFINE l_time DATETIME HOUR TO SECOND
...
-- Update the status in the database
	TRY
		UPDATE alert_status SET (state, state_desc, last_update_on, last_update_at) = ("O", "Okay", l_date, l_time)
				WHERE controller_name = m_controller
	CATCH
	END TRY
	IF STATUS != 0 THEN
...
PRIVATE FUNCTION service_reply(l_reply STRING) RETURNS STRING
...
--		DROP TABLE alert_status
	...
	IF STATUS != 0 THEN
		...
		CREATE TABLE alert_status(
				controller_name VARCHAR(254), state CHAR(1), state_desc VARCHAR(40), last_update_on DATE,
	...
	CATCH
		IF STATUS != 0 THEN
			...
			CREATE TABLE alert_messages(
					machine_name VARCHAR(254), alert_type VARCHAR(20), message VARCHAR(254), recieved_on DATE,
